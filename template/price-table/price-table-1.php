							...
							if($the_query->found_posts>0){
								...
								foreach ( $membership_pack as $row )
								{
									...
									if($recurring == 'on'){
										...
										$recurring_text=__(' Per ' ,'epfitness').' '.__(get_post_meta($row->ID, 'ep_fitness_package_recurring_cycle_type', true),'epfitness');
										}else{
										$recurring_text=__(' Per ' ,'epfitness').$count_arb.' '.__(get_post_meta($row->ID, 'ep_fitness_package_recurring_cycle_type', true).'s' ,'epfitness');
										}
									...
									?><div class="col-md-4"><ul id="pt<?php echo $pt;?>" >
									    ...
									    <div class="submit-btn"> <a href="<?php echo get_page_link($page_name_reg).'?&package_id=	'.$row->ID ; ?>"><?php  _e('Sign up','epfitness');?></a> </div>
									 </li>
									</ul></div><?php
								$i++;
								}
