							...
							<div id="paypal-button" class="margin-top-20">
								<?php
								 ...
								 if($package_name!="" AND $p_amount=='0' ){ ?>
									...
									<button  id="submit_ep_fitness_payment" name="submit_ep_fitness_payment"  type="submit" class="btn-new btn-custom ctrl-btn"  > <?php  _e('Submit','epphotographer');?></button>

								<?php
								}else{
									?>
									<div id="loading-3" style="display: none;"><img src='<?php echo wp_ep_fitness_URLPATH. 'admin/files/images/loader.gif'; ?>' /></div>
								<button  id="submit_ep_fitness_payment" name="submit_ep_fitness_payment" type="submit" class="btn btn-info ctrl-btn"  ><?php  _e('Submit','epphotographer');?>  </button>
